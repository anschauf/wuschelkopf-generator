lime = 'lime'
orange = 'orange'
grey = 'grey'
blue = 'blue'
burning = 'burning'
shiningPurple = 'ShiningPurple'
brown = 'brown'
red = 'red'
black = 'black'

lightSkin = 'light'
skeletonSkin = 'Skeleton'
chocoSkin = 'choco'
ghostSkin = 'Ghost'

hero_male = [
    '00_Background\\Purple.png',
    f'01_Body\\SKIN_Standard_{lightSkin}.png',
    f'02_Clothes\\IsSkeletive_HoodieRaspberry_Basic.png',
    f'03_Shoes\\ConverseMint.png',
    f'04_Hair\\HAIR_Standard_{lime}.png',
    None,
    '06_Mouth\\IsAlive_BigSmile_Basic.png',
    '07_Eyes\\IsAlive_Bow_Basic.png',
    None,
    None,
    '10_Hats\\CrownDefault.png',
    None,
    f'12_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{lightSkin}.png',
    None,
    f'14_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{lightSkin}.png',
    f'15_Righthand-Back\\SKIN_FLATUP_{lightSkin}.png',
    '16_Righthand-Thing\\MULTI-SKIN_FrogAlien_FLATUP.png',
    None,
]

hero_female = [
    '00_Background\\Blue.png',
    f'01_Body\\SKIN_Standard_{skeletonSkin}.png',
    '02_Clothes\\IsNakedShoes_RubberduckLimegreen.png',
    f'03_Shoes\\HasNakedShoes_FlipflopSalmon_OpenSkeleton.png',
    f'04_Hair\\HAIR_Standard_{orange}.png',
    None,
    None,
    None,
    f'08_HairExtensions\\HAIR_Pigtail_Orange.png',
    None,
    f'10_Headdeko\\FlowerYellow.png',
    f'11_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    '12_Lefthand-Thing\\MULTI-SKIN_TeddyWhite_HOLDSTRAIGHT.png',
    f'13_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    f'14_Righthand-Back\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    '15_Righthand-Thing\\MULTI-SKIN_WindwheelYellowPurple_HOLDSTRAIGHT.png',
    f'16_Righthand-Front\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png'
]

skeleton_joker = [
    '00_Background\\Orange.png',
    f'01_Body\\SKIN_Standard_{skeletonSkin}.png',
    f'02_Clothes\\JokerYellowPurple.png',
    f'03_Shoes\\JokerYellowPurple.png',
    f'04_Hair\\HAIR_Standard_{grey}.png',
    f'05_Chindeko\\HAIR_Beard_{grey}.png',
    None,
    '06_2_Skeleton_Mouth\\BlingGold.png',
    None,
    '07_2_Skeleton_Eyes\\Rubinleft.png',
    None,
    None,
    '10_Hats\\JokerYellowPurple.png',
    f'11_Headdeko\\EarringsOneWhitegold.png',
    f'12_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    None,
    f'14_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    f'15_Righthand-Back\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    '16_Righthand-Thing\\MULTI-SKIN_LollipopGreen_HOLDSTRAIGHT.png',
    f'17_Righthand-Front\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png'
]

choco_mage_female = [
    '00_Background\\Lightblue.png',
    f'01_Body\\SKIN_Standard_{chocoSkin}.png',
    '02_Clothes\\JediWhite.png',
    f'03_Shoes\\HasNakedShoes_Barefoot_Closed{chocoSkin}.png',
    f'04_Hair\\HAIR_Standard_{shiningPurple}.png',
    '05_Mouth\\IsAlive_Fangs_Basic.png',
    '06_Eyes\\IsAlive_ShiningPurple_Basic.png',
    '07_Facedeko\\Tattoo.png',
    None,
    None,
    f'10_Headdeko\\SKIN_ElvenearsAndHornsWhite_{chocoSkin}.png',
    f'11_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    '12_Lefthand-Thing\\MULTI-SKIN_TurtleAlbino_HOLDSTRAIGHT.png',
    f'13_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    f'14_Righthand-Back\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    '15_Righthand-Thing\\MULTI-SKIN_MagicWandWhite_HOLDSTRAIGHT.png',
    f'16_Righthand-Front\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png'
]

joint_adam = [
    '00_Background\\Red.png',
    f'01_Body\\SKIN_Standard_{chocoSkin}.png',
    f'02_Clothes\\IsHairNakedShoes_Adam_Blue.png',
    f'03_Shoes\\UggbootsBrown.png',
    f'04_Hair\\HAIR_Standard_{blue}.png',
    f'05_Chindeko\\HAIR_BeardBraid_{blue}.png',
    f'06_Mouth\\IsAlive_OneTooth_Basic.png',
    f'07_Eyes\\IsAlive_Stoned_Basic.png',
    None,
    None,
    None,
    f'11_Headdeko\\AlienYellow.png',
    f'12_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    '13_Lefthand-Thing\\MULTI-SKIN_GhettoblasterPurple_HOLDSTRAIGHT.png',
    f'14_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    None,
    '16_Righthand-Thing\\SKIN_Joint_choco.png',
    None,
]

ghost_rider = [
    '00_Background\\Grey.png',
    f'01_Body\\SKIN_Standard_{skeletonSkin}.png',
    f'02_Clothes\\BikerDefault.png',
    f'03_Shoes\\ConverseBlack.png',
    f'04_Hair\\HAIR_Standard_{burning}.png',
    f'05_Chindeko\\HAIR_BeardBraid_{burning}.png',
    None,
    None,
    f'08_Facedeko\\EyepatchBlack.png',
    None,
    None,
    None,
    f'12_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    f'13_Lefthand-Thing\\MULTI-SKIN_WalkingstickDark_HOLDSTRAIGHT.png',
    f'14_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{skeletonSkin}.png',
    None,
    '16_Righthand-Thing\\SKIN_ThumbUp_Skeleton.png',
    None,
]

jedi_ghost = [
    '00_Background\\Purple.png',
    f'01_Body\\SKIN_Standard_{ghostSkin}.png',
    '02_Clothes\\JediBrown.png',
    f'03_Shoes\\PiratebootsBrown.png',
    f'04_Hair\\HAIR_Standard_{brown}.png',
    f'05_Mouth\\IsAlive_LipsNude_Ghost.png',
    f'06_Eyes\\IsAlive_ShiningBlack_Ghost.png',
    None,
    f'08_HairExtensions\\HAIR_Pigtail_{brown}.png',
    None,
    f'10_Headdeko\\SafetyglassesBeige.png',
    f'11_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png',
    None,
    f'13_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png',
    f'14_Righthand-Back\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png',
    f'15_Righthand-Thing\\SKIN_Greensaber_{ghostSkin}.png',
    f'16_Righthand-Front\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png'
]

fairy = [
    '00_Background\\Orange.png',
    f'01_Body\\SKIN_Standard_{chocoSkin}.png',
    '02_Clothes\\IsNakedShoes_WizardWhite.png',
    f'03_Shoes\\HasNakedShoes_HighHeelsWhite_Openchoco.png',
    f'04_Hair\\HAIR_Standard_{lime}.png',
    f'05_Mouth\\IsAlive_Vampire_Basic.png',
    f'06_Eyes\\IsAlive_LashedSilver_Basic.png',
    None,
    None,
    None,
    f'10_Headdeko\\SKIN_ElvenearsAndHornsWhite_choco.png',
    f'11_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    f'12_Lefthand-Thing\\MULTI-SKIN_ElephantBlue_HOLDSTRAIGHT.png',
    f'13_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    f'14_Righthand-Back\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png',
    f'15_Righthand-Thing\\MULTI-SKIN_LeviosaWhite_HOLDSTRAIGHT.png',
    f'16_Righthand-Front\\SKIN_HOLDSTRAIGHT_{chocoSkin}.png'
]

drunk_lepricon = [
    '00_Background\\Red.png',
    f'01_Body\\SKIN_Standard_{lightSkin}.png',
    f'02_Clothes\\SuitGreen.png',
    f'03_Shoes\\PiratebootsBlack.png',
    f'04_Hair\\HAIR_Standard_{red}.png',
    f'05_Chindeko\\HAIR_Beard_{red}.png',
    f'06_Mouth\\IsAlive_OneTooth_Basic.png',
    f'07_Eyes\\IsAlive_HeartsGreen_Basic.png',
    f'08_Facedeko\\MonocleGold.png',
    None,
    f'10_Hats\\LepriconDefault.png',
    None,
    f'12_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{lightSkin}.png',
    f'13_Lefthand-Thing\\MULTI-SKIN_GuitarRed_HOLDSTRAIGHT.png',
    f'14_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{lightSkin}.png',
    f'15_Righthand-Back\\SKIN_HOLDSTRAIGHT_{lightSkin}.png',
    '16_Righthand-Thing\\MULTI-SKIN_BottleGreen_HOLDSTRAIGHT.png',
    f'17_Righthand-Front\\SKIN_HOLDSTRAIGHT_{lightSkin}.png'
]

illidan = [
    '00_Background\\Blue.png',
    f'01_Body\\SKIN_Standard_{ghostSkin}.png',
    f'02_Clothes\\TrousersDark.png',
    f'03_Shoes\\PiratebootsBlack.png',
    f'04_Hair\\HAIR_Standard_{black}.png',
    None,
    f'06_Mouth\\IsAlive_Fangs_Ghost.png',
    f'07_Eyes\\IsAliveNoFaceDeco_IllidanGreen_Ghost.png',
    f'08_Facedeko\\Tattoo.png',
    None,
    None,
    f'11_Headdeko\\SKIN_ElvenearsAndHornsBlack_Ghost.png',
    f'12_Lefthand-Back\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png',
    f'13_Lefthand-Thing\\MULTI-SKIN_KnockedOutOrange_HOLDSTRAIGHT.png',
    f'14_Lefthand-Front\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png',
    f'15_Righthand-Back\\SKIN_HOLDSTRAIGHT_{ghostSkin}.png',
    '16_Righthand-Thing\\MULTI-SKIN_BookBlack_RIGHTSIDE.png',
    f'17_Righthand-Front\\SKIN_RIGHTSIDE_{ghostSkin}.png'
]

# paths, is_female, skin_tones, hair_color
specific_heroes = {
    5: (hero_male, False, skeletonSkin, orange.capitalize()),
    23: (hero_female, True, skeletonSkin, orange.capitalize()),
    3: (skeleton_joker, False, skeletonSkin, grey.capitalize()),
    17: (choco_mage_female, True, chocoSkin, shiningPurple.capitalize()),
    32: (joint_adam, False, chocoSkin, blue.capitalize()),
    24: (ghost_rider, False, skeletonSkin, burning.capitalize()),
    29: (jedi_ghost, True, ghostSkin, brown.capitalize()),
    35: (fairy, True, chocoSkin, lime.capitalize()),
    41: (drunk_lepricon, False, lightSkin, red.capitalize()),
    65: (illidan, False, ghostSkin, black.capitalize()),
}
